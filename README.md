# Kampus Tangerang App 🎓

Aplikasi Android sederhana yang menampilkan daftar perguruan tinggi ternama di wilayah **Tangerang** dan **Tangerang Selatan**. Aplikasi ini dirancang untuk memudahkan calon mahasiswa melihat informasi singkat dan visual dari berbagai kampus di area tersebut.

## 🚀 Fitur Utama

- **Daftar Kampus Lengkap**: Menampilkan berbagai universitas besar seperti UMN, UPH, BINUS, UNPAM, dan lainnya.
- **Visual Modern**: Menggunakan `CardView` dan `Material Design` untuk tampilan yang bersih dan intuitif.
- **Gambar Online**: Mengintegrasikan library `Glide` untuk memuat foto gedung kampus secara real-time dari internet.
- **Informasi Singkat**: Menyediakan nama, lokasi spesifik, dan deskripsi singkat mengenai fokus pendidikan setiap kampus.

## 🛠️ Teknologi yang Digunakan

- **Bahasa**: [Kotlin](https://kotlinlang.org/)
- **UI Framework**: Android Jetpack (ViewBinding, RecyclerView, ConstraintLayout)
- **Image Loading**: [Glide](https://github.com/bumptech/glide)
- **Minimum SDK**: API 24 (Android 7.0)
- **Target SDK**: API 34 (Android 14)

## 📸 Tampilan Aplikasi

Aplikasi menampilkan daftar dalam bentuk kartu yang berisi:
1. Foto Kampus
2. Nama Universitas
3. Lokasi (Kabupaten/Kota)
4. Deskripsi singkat

## ⚙️ Cara Menjalankan Proyek

1. **Clone Repository**:
   ```bash
   git clone https://github.com/username/KampusTangerang.git
   ```
2. **Buka di Android Studio**:
   Pilih "Open" dan arahkan ke folder proyek hasil clone.
3. **Build & Run**:
   Pastikan perangkat atau emulator Anda terhubung ke internet untuk memuat gambar kampus.

## 📄 Lisensi
Proyek ini dibuat untuk tujuan pembelajaran. Silakan gunakan dan kembangkan lebih lanjut!
